...
loadHelpData; //this procedure will read text from customerhelp.txt and place it in richedit for
//user to read when they click to help tab.
end;
...
if FileExists(ExtractFilePath(ParamStr(0))+defaultImage) then
  begin
    ...
    Image2.Picture.Graphic.LoadFromFile(ExtractFilePath(ParamStr(0))+ defaultImage);
    Image1.Picture.Graphic.LoadFromFile(ExtractFilePath(ParamStr(0))+ defaultImage);
  end
else
...
Label4.Caption := 'Name : '+lblName.Caption;
Label5.Caption := 'Surname : '+lblSurname.Caption;
...
procedure TForm24.loadHelpData;
var txt : textfile;
    ln : string;
begin
//
AssignFile(txt, ExtractFilePath( ParamStr(0))+'dataSources\Customerhelp.txt');
RichEdit1.Clear;

RichEdit1.SelAttributes.Name := 'Takion';
RichEdit1.SelAttributes.Size := 30;
RichEdit1.SelAttributes.Style := [fsBold, fsUnderline];

RichEdit1.Lines.Add('HELP DESK.');
if FileExists(ExtractFilePath( ParamStr(0))+'dataSources\Customerhelp.txt') then
  begin
    Reset(txt);
...
if showing1 = True then
    begin
      ...
      Image4.Picture.LoadFromFile(ExtractFilePath(ParamStr(0))+'media\icons\lock-128.png');
    end
else
    begin
      Showing1 := True;
      edtAcc.PasswordChar := 'X';
      Image4.Picture.LoadFromFile(ExtractFilePath(ParamStr(0))+'media\icons\unlocked-128.png');
    end;
end;
...
if Showing2 = True then
    begin
      ...
      Image3.Picture.LoadFromFile(ExtractFilePath(ParamStr(0))+'media\icons\lock-128.png');
    end
else
    begin
      Showing2 := True;
      edtPass.PasswordChar := '•';
      Image3.Picture.LoadFromFile(ExtractFilePath(ParamStr(0))+'media\icons\unlocked-128.png');
    end;
end;
